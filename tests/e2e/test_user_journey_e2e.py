...
class TestUserJourneyE2E:
    ...
    async def test_new_user_complete_journey(self, client: AsyncClient):
        ...
        assert register_response.status_code in (200, 400), f"Registration failed: {register_response.text}"
        ...
        # 4. Update first task
        update_data = TestDataFactory.create_task_update_data(title="Updated Journey Task 1")
        response = await client.patch(f"/tasks/{task_ids[0]}", json=update_data, headers=headers)
        ...
        updated_task = next(t for t in tasks if t["id"] == task_ids[0])
        completed_task = next(t for t in tasks if t["id"] == task_ids[1])
    ...
    async def test_multi_session_consistency(self, client: AsyncClient):
        ...
        response = await client.patch(f"/tasks/{task_id}", json=update_data, headers=headers2)
        assert response.status_code == 200
